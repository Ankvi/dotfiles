...
export async function installWowUp() {
    ...
    console.log(`Found WowUp.CF version: ${version}, created at ${releaseDate}`);
    ...
    if (await exists(installPath)) {
        ...
        if (!appImageExists) {
            console.log(`Could not find app image at: ${downloadPath}. Downloading from GitHub...`);
            await Bun.spawn([
                "gh",
                "release",
                "download",
        ...
        if (!appImageExists) {
            console.error("Couldn't find any app images in download folder. Exiting");
            exit(1);
        }
